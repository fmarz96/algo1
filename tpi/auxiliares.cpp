...
// EJERCICIO 11
int indiceDelMaximoK(vector<int> ticksHastaMorir) {
    int max = ticksHastaMorir[0];
    int indiceMaximo = 0;
    for(int i = 1; i < ticksHastaMorir.size(); i++){
        if(max < ticksHastaMorir[i]){
            max = ticksHastaMorir[i];
            indiceMaximo = i;
        }
    }
    return indiceMaximo;
